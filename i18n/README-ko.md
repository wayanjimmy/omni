...
|-----------------|-----------|-----------|-----------|------------------|
| `docker build` (다단계) | 9.2 KB | 49 bytes | **99.5%** | 캐싱 노이즈를 제거합니다; AI는 실제 빌드 오류를 즉시 확인합니다. |
| `cargo test` (대규모 제품군) | 16.5 KB | 4.3 KB | **78.0%** | 수백 개의 "ok" 테스트를 제거합니다; AI는 실패와 스택 추적에만 집중합니다. |
| `git status` (dirty) | 496 bytes | 113 bytes | **77.2%** | 깨끗한 파일과 힌트를 제거하고 수정/추적되지 않은 파일만 유지합니다. |
| `kubectl get pods` | 840 bytes | 762 bytes | **10.0%** | CrashLoopBackOff/Error 포드를 선택적으로 표시하고 정상 포드를 건너뜁니다. |
| `git diff` (다중 파일) | 397 bytes | 220 bytes | **50.0%** | 변경 사항이 있는 헝크를 유지하고 과도한 컨텍스트 줄을 삭제합니다. |

- **파이프라인 지연 시간**: **< 100ms** (바이너리 시작을 포함한 종단간)
- **전체 절감액**: 평균 개발 세션에서 토큰 **97.3%** 감소.
- **ROI**: 개발자당 월 **$35+ USD** 절약(플래그십 모델 기준).

*자신의 실제 토큰 절약량을 확인하려면 며칠 사용 후 `omni stats`를 실행하기만 하면 됩니다.*
