...
- **结构化的 ReadFile + Grep**：OMNI不返回原始文件转储或平面grep输出，而是返回结构化的大纲（导入，公共API，风险标记）和分组的grep摘要（按匹配次数排序的顶级文件，优先显示优先级行）。
- **基于事实的抗幻觉守卫**：OMNI仅在掌握确凿事实时发出警告——没有推测。如果输出被严重压缩并且不存在回溯：它会说明。如果一个文件有许多依赖项：它会说明。让您的AI脚踏实地。
