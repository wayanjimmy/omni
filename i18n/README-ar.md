...
|----------------|-------------|-------------|--------------|------------------------------|
| `docker build` (متعدد المراحل) | 9.2 KB | 49 bytes | **99.5%** | يقضي على ضوضاء التخزين المؤقت؛ يرى الذكاء الاصطناعي خطأ البناء الحقيقي على الفور. |
| `cargo test` (مجموعة كبيرة) | 16.5 KB | 4.3 KB | **78.0%** | يزيل المئات من اختبارات "ok"؛ يركز الذكاء الاصطناعي فقط على حالات الفشل وتتبع المكدس. |
| `git status` (dirty) | 496 bytes | 113 bytes | **77.2%** | يزيل الملفات النظيفة والتلميحات؛ يحتفظ فقط بالملفات المعدلة/غير المتتبعة. |
| `kubectl get pods` | 840 bytes | 762 bytes | **10.0%** | يبرز بشكل انتقائي كبسولات CrashLoopBackOff/Error، متخطياً السليمة منها. |
| `git diff` (ملفات متعددة) | 397 bytes | 220 bytes | **50.0%** | يحافظ على أجزاء التعليمات البرمجية التي تحتوي على تغييرات، مع إسقاط أسطر السياق الزائدة. |

- **زمن انتقال خط الأنابيب**: **< 100ms** (من البداية إلى النهاية، بما في ذلك بدء تشغيل الثنائي)
- **إجمالي التوفير في كل الأوقات**: تقليل الرموز بنسبة **97.3%** عبر جلسات التطوير المتوسطة.
- **عائد الاستثمار (ROI)**: توفير **$35+ USD** لكل مطور/شهر (مُقاساً مقابل النماذج الرئيسية).

*لرؤية توفير الرموز الفعلي الخاص بك، ما عليك سوى تشغيل `omni stats` بعد بضعة أيام من الاستخدام.*
