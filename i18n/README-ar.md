...
- **قراءة منظمة للملفات (ReadFile) + Grep**: بدلاً من تفريغ الملفات الخام أو إخراج grep المسطح، يُرجع OMNI مخططات منظمة (الواردات، وواجهة برمجة التطبيقات العامة، وعلامات المخاطر) وملخصات grep مجمعة (أهم الملفات حسب عدد التطابق، أسطر الأولوية أولاً).
- **حراس مكافحة الهلوسة الواقعية**: يصدر OMNI تحذيرات فقط عندما تكون لديه حقائق ثابتة - لا توجد تكهنات. إذا كان الإخراج مضغوطًا بشدة ولا يوجد ترجيع: فهو يقول ذلك. إذا كان الملف يحتوي على العديد من التبعيات: يقول ذلك. الحفاظ على الذكاء الاصطناعي الخاص بك مرتكزًا على الواقع.
