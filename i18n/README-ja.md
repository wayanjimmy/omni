...
|-------------------------|------------|------------|--------------|------------|
| `docker build` (マルチステージ) | 9.2 KB | 49 bytes | **99.5%** | キャッシュノイズを排除；AIは実際のビルドエラーを即座に確認できます。 |
| `cargo test` (大規模スイート) | 16.5 KB | 4.3 KB | **78.0%** | 何百もの「ok」テストを剥ぎ取ります；AIは失敗とスタックトレースのみに集中します。 |
| `git status` (dirty) | 496 bytes | 113 bytes | **77.2%** | クリーンなファイルとヒントを削除；変更/未追跡ファイルのみを保持します。 |
| `kubectl get pods` | 840 bytes | 762 bytes | **10.0%** | CrashLoopBackOff/Errorポッドを選択的に表示し、正常なポッドをスキップします。 |
| `git diff` (複数ファイル) | 397 bytes | 220 bytes | **50.0%** | 変更のあるハンクを保持し、過剰なコンテキスト行を削除します。 |

- **パイプライン遅延**: **< 100ms**（バイナリ起動を含むエンドツーエンド）
- **これまでの総節約**: 平均的な開発セッション全体で **97.3%** のトークン削減。
- **ROI**: 開発者1人あたり月額 **$35+ USD** 節約（フラッグシップモデルで測定）。

*実際のトークン節約量を確認するには、数日間使用した後に `omni stats` を実行するだけです。*
